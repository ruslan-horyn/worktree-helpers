...
- [ ] After a successful update, `wt --update` prints an exact `source <path>/wt.sh` command to stdout
- [ ] The path shown is `$_WT_DIR/wt.sh` (the actual install path, not a placeholder)
- [ ] The re-source block is printed to stdout (not stderr)
- [ ] The prompt includes a secondary hint: "Or open a new terminal."
- [ ] If already on the latest version, no re-source prompt is shown
- [ ] If the update fails (network error, clone failure), no re-source prompt is shown
- [ ] `wt --update --check` (check-only mode) is not affected; no re-source prompt shown
- [ ] `shellcheck` passes on the modified `lib/update.sh`
- [ ] Existing BATS tests in `test/cmd_update.bats` continue to pass
- [ ] New BATS test asserts the re-source prompt appears in `_update_install` output after a successful update
- [ ] New BATS test asserts no re-source prompt when versions match
...
- [ ] Re-source prompt added to the success path of `_update_install` in `lib/update.sh`
- [ ] Prompt uses `$_WT_DIR` to show the exact install path
- [ ] Prompt printed to stdout; remaining messages remain on stderr via `_info`
- [ ] Fallback message shown if `_WT_DIR` is empty
- [ ] `shellcheck lib/update.sh` passes (no new warnings)
- [ ] Two new BATS tests written in `test/cmd_update.bats` and passing
- [ ] All existing `test/cmd_update.bats` tests continue to pass
- [ ] Conventional commit used: `feat(STORY-033): ...`
